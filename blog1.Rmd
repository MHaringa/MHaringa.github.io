...
- US 3m TR had a statistically significant coefficient `r data.frame(name = dimnames(bootstrap_procedure[[3]])[[1]], waarde = as.numeric(bootstrap_procedure[[3]])) %>% dplyr::filter(name == "US.3m.TR") %>% dplyr::select(waarde) %>% as.numeric(.)`% of the time
