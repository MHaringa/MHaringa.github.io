...
#function to create map with categorized legend scale
#scale_fill_distiller(palette = "RdYlGn") works easier: no need for cut continuous into categorical values
RedYellowGreen <- rev(brewer.pal(5, 'RdYlGn'))
RedYellowGreen7 <- rev(brewer.pal(7, 'RdYlGn'))
RedYellowGreen1 <- brewer.pal(5, 'RdYlGn')[3:5]
...
fisher <- function(data){
  classIntervals(data, n=5, style='fisher', intervalClosure='right')[[2]]
}
...
finaldata0 <- finaldata %>% 
  ...
  mutate(NCLAIMS.cut = cut(NCLAIMS, breaks = fisher(NCLAIMS), include.lowest = TRUE)) %>%
  mutate(EXP.cut = cut(EXP, breaks = fisher(EXP), include.lowest = TRUE, dig.lab = 4)) 
